...
(function(){
	var iaHeaderNav = {
		...
		l2To3Handeler : function(e) { // On Clicking L2 Items activating menu if menu is available or redirecting to link location
			...
			if($hasSubNav) {
				if((e.type=="keyup" && e.keyCode==13 && window.innerWidth>=1024) || (e.type=="click")) {
...
$(document).ready(function(){
	...
	$(".iah__l4-column-holder").each(function(){
        const $title = $(this).find('.iah__l4-column-title');
		const listCount = $(this).find('ol li').length;
        const $firstListItem = $(this).find('ol li:first-child a');
        $firstListItem.attr('aria-label',`Grouping ${$title.text().trim()} list with ${listCount} items ${$firstListItem.text().trim()}`);
 		$(this).find('ol li a').on('focus', function(){
			const text = $(this).text().trim();
			if($(this).is($firstListItem)){
				$(this).attr('aria-label',`Grouping ${$title.text().trim()}	list with ${listCount} items ${$firstListItem.text().trim()}`);
           }else {
		...
		$firstListItem.on('blur', function(){
			$(this).attr('aria-label',`Grouping ${$title.text().trim()} list with ${listCount} items ${$(this).text().trim()}`);
		});
	});
